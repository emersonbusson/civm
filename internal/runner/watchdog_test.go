...
func TestWatchdogRestartsFailedOrgRunnerWhenReposCannotBeInferred(t *testing.T) {
	...
	opts.SystemRunnersFn = func(context.Context) ([]Status, error) {
		return []Status{{
			UnitName:    "actions.runner.acme.civm-acme-org.service",
			Repo:        "acme",
			Name:        "civm-acme-org",
			ActiveState: "failed",
			SubState:    "failed",
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		calls = append(calls, call)
		switch call {
		case "systemctl show actions.runner.acme.civm-acme-org.service --property=WorkingDirectory --value":
			return []byte("/home/runner/actions-runner-acme-org\n"), nil
		case "systemctl is-active actions.runner.acme.civm-acme-org.service":
			return []byte("active\n"), nil
		default:
			return nil, nil
		}
	...
	opts.ReadFileFn = func(path string) ([]byte, error) {
		...
		return append([]byte{0xef, 0xbb, 0xbf}, []byte(`{"gitHubUrl":"https://github.com/acme"}`)...), nil
	}
	...
	assertWatchdogCall(t, calls, "sudo systemctl restart actions.runner.acme.civm-acme-org.service")
	if !hasWatchdogEvent(report, "runner-restarted") {
		t.Fatalf("events = %+v, want runner-restarted", report.Events)
	}
...
func TestWatchdogOrgBusyRestartHasOneHourCooldown(t *testing.T) {
	...
	opts.SystemRunnersFn = func(context.Context) ([]Status, error) {
		return []Status{{
			UnitName: "actions.runner.acme.civm-acme-org.service", Repo: "acme",
			Name: "civm-acme-org", ActiveState: "active", SubState: "running",
		}}, nil
	}
	restarts := 0
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "systemctl show actions.runner.acme.civm-acme-org.service --property=WorkingDirectory --value":
			return []byte("/home/runner/actions-runner-acme-org\n"), nil
		case "gh api /orgs/acme/actions/runners":
			return []byte(`{"runners":[{"name":"civm-acme-org","status":"online","busy":true}]}`), nil
		case "sudo systemctl restart actions.runner.acme.civm-acme-org.service":
			restarts++
			return nil, nil
		case "systemctl is-active actions.runner.acme.civm-acme-org.service":
			return []byte("active\n"), nil
	...
	opts.ReadFileFn = func(path string) ([]byte, error) {
		...
		if strings.HasSuffix(path, "/.runner") {
			return []byte(`{"gitHubUrl":"https://github.com/acme"}`), nil
		}
		return nil, os.ErrNotExist
	}
...
func TestWatchdogSkipsNoRepoRestartWhenWorkerAppears(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		call := name + " " + strings.Join(args, " ")
		calls = append(calls, call)
		if call == "systemctl show actions.runner.acme.civm-acme-org.service --property=WorkingDirectory --value" {
			return []byte("/home/runner/actions-runner-acme-org\n"), nil
		}
		return nil, nil
	}
...
func TestWatchdogRestartsOnlineIdleOrgRunnerWithPersistentCivmQueueOnce(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "gh api /orgs/acme/actions/runners":
			return []byte(fmt.Sprintf(
				`{"runners":[{"id":1,"name":"civm-acme-org","status":"online","busy":%t,"labels":[{"name":"self-hosted"},{"name":"civm"}]}]}`,
				runnerBusy,
			)), nil
		default:
			return []byte("active\n"), nil
	...
	opts.RestartFn = func(_ context.Context, unit string) error {
		if unit != "actions.runner.acme.civm-acme-org.service" {
	...
	if strings.Join(first.Repos, ",") != "acme/app" {
	...
	if _, exists := state.QueueStalls["actions.runner.acme.civm-acme-org.service"]; exists {
...
func TestWatchdogQueueStallFailsClosedWhenIdleUnknown(t *testing.T) {
	...
	opts.QueueReposFn = func() ([]string, error) { return []string{"acme/app"}, nil }
	opts.QueuedCivmJobsFn = func(
		context.Context,
		[]string,
		time.Time,
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "systemctl show actions.runner.acme.civm-acme-org.service --property=WorkingDirectory --value":
			return []byte("/home/runner/actions-runner-acme-org\n"), nil
		case "gh api /orgs/acme/actions/runners":
			return []byte(`{"runners":[{"name":"civm-acme-org","status":"online","busy":false,"labels":[{"name":"self-hosted"},{"name":"civm"}]}]}`), nil
		default:
			return nil, nil
		}
	}
...
func TestListQueuedCivmJobsPaginatesFiltersAndSignsOldest(t *testing.T) {
	...
	runFn := func(_ context.Context, name string, args ...string) ([]byte, error) {
		call := name + " " + strings.Join(args, " ")
		switch call {
		case "gh api --paginate --slurp /repos/acme/app/actions/runs?per_page=100&status=queued":
		...
		case "gh api --paginate --slurp /repos/acme/app/actions/runs/10/jobs?filter=latest&per_page=100":
			return []byte(`[{"jobs":[
		...
		case "gh api --paginate --slurp /repos/acme/app/actions/runs/11/jobs?filter=latest&per_page=100":
			return []byte(`[{"jobs":[{"id":102,"status":"in_progress","labels":["self-hosted","civm"]}]}]`), nil
		case "gh api --paginate --slurp /repos/acme/docs/actions/runs?per_page=100&status=queued":
			return []byte(`[{"workflow_runs":[
			  {"id":20,"status":"queued","created_at":"2026-05-19T11:40:00Z"}
			]}]`), nil
		case "gh api --paginate --slurp /repos/acme/docs/actions/runs/20/jobs?filter=latest&per_page=100":
	...
	oldestIdentity := "acme/docs/00000000000000000020/00000000000000000200"
...
func TestDefaultWatchdogQueueReposValidatesAndDeduplicates(t *testing.T) {
	t.Setenv("CIVM_REAPER_REPOS", "acme/docs, acme/app,acme/docs")
	got, err := defaultWatchdogQueueRepos()
	if err != nil {
		t.Fatalf("defaultWatchdogQueueRepos: %v", err)
	}
	want := []string{"acme/app", "acme/docs"}
...
func TestNormalizeWatchdogRepos(t *testing.T) {
	t.Parallel()
	got, err := normalizeWatchdogRepos([]string{" emersonbusson/civm ", "acme/app", "emersonbusson/civm", ""})
	if err != nil {
		t.Fatalf("normalizeWatchdogRepos: %v", err)
	}
	if strings.Join(got, ",") != "acme/app,emersonbusson/civm" {
		t.Fatalf("repos=%v, want sorted unique fleet", got)
	}
	if _, err := normalizeWatchdogRepos([]string{"acme"}); err == nil {
...
func TestWatchdogRestartsOrgRunnerBusyWithoutLocalWorker(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "systemctl show actions.runner.acme.civm-acme-org.service --property=WorkingDirectory --value":
			return []byte("/home/runner/actions-runner-acme-org\n"), nil
		case "gh api /orgs/acme/actions/runners":
			return []byte(`{"runners":[{"id":1,"name":"civm-acme-org","status":"online","busy":true,"labels":[{"name":"self-hosted"},{"name":"civm"}]}]}`), nil
		case "systemctl is-active actions.runner.acme.civm-acme-org.service":
			return []byte("active\n"), nil
		default:
			return nil, nil
...
func TestDefaultWatchdogRestartFreezesBeforeFinalWorkerProbe(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		call := name + " " + strings.Join(args, " ")
		switch call {
		case "sudo systemctl freeze actions.runner.acme.civm-acme-org.service":
			frozen = true
			return nil, nil
		case "sudo systemctl thaw actions.runner.acme.civm-acme-org.service":
			frozen = false
			thaws++
			return nil, nil
		case "sudo systemctl restart actions.runner.acme.civm-acme-org.service":
	...
	err := defaultWatchdogRestart(
		context.Background(),
		opts,
		"actions.runner.acme.civm-acme-org.service",
	)
	if err == nil {
...
func TestDefaultWatchdogRestartFreezesProbeThenStopsGracefully(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "sudo systemctl freeze actions.runner.acme.civm-acme-org.service":
			frozen = true
		case "sudo systemctl stop actions.runner.acme.civm-acme-org.service":
			if !frozen {
	...
	want := []string{
		"sudo systemctl freeze actions.runner.acme.civm-acme-org.service",
		"sudo systemctl stop actions.runner.acme.civm-acme-org.service",
		"sudo systemctl kill --kill-who=all --signal=SIGKILL actions.runner.acme.civm-acme-org.service",
		"sudo systemctl restart actions.runner.acme.civm-acme-org.service",
		"systemctl is-active actions.runner.acme.civm-acme-org.service",
	}
...
func TestDefaultWatchdogRestartPurgesFailedUnitCgroupBeforeRestart(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "systemctl is-active actions.runner.acme.civm-acme-org.service":
			if slices.Contains(calls, "sudo systemctl restart actions.runner.acme.civm-acme-org.service") {
				return []byte("active\n"), nil
			}
			return []byte("failed\n"), errors.New("exit status 3")
		}
	...
	kill := slices.Index(calls, "sudo systemctl kill --kill-who=all --signal=SIGKILL actions.runner.acme.civm-acme-org.service")
	restart := slices.Index(calls, "sudo systemctl restart actions.runner.acme.civm-acme-org.service")
...
func TestDefaultWatchdogRestartFailsClosedWhenCgroupPurgeFails(t *testing.T) {
	...
	opts.RunFn = func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "systemctl is-active actions.runner.acme.civm-acme-org.service":
			return []byte("failed\n"), errors.New("exit status 3")
		case "sudo systemctl kill --kill-who=all --signal=SIGKILL actions.runner.acme.civm-acme-org.service":
			return nil, errors.New("access denied")
		case "systemctl show actions.runner.acme.civm-acme-org.service --property=ControlGroup --value":
			return []byte("/system.slice/actions.runner.acme.civm-acme-org.service\n"), nil
		case "sudo systemctl restart actions.runner.acme.civm-acme-org.service":
			restarts++
		}
		return nil, nil
	...
	opts := WatchdogOptions{
		RunFn: func(_ context.Context, name string, args ...string) ([]byte, error) {
			...
			if name == "gh" && strings.Join(args, " ") == "api /repos/acme/civm/actions/runs?per_page=20&status=completed" {
				return []byte(`{"workflow_runs":[{"id":8,"head_sha":"abc","status":"completed","conclusion":"failure","created_at":"2026-05-19T12:00:00Z","html_url":"https://github.com/emersonbusson/civm/actions/runs/8","pull_requests":[{"number":7}]}]}`), nil
			}
