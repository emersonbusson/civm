...
func TestExecRunnerRejectsDigestDriftAndUnsafeInputs(t *testing.T) {
	...
	request.DriverExecutable = driver
	request.DriverSHA256 = digest
	if err := os.WriteFile(driver, []byte("#!/bin/sh\nexit 7\n"), 0o700); err != nil { //nolint:gosec // test driver script needs execute bits
		t.Fatal(err)
	}
...
func TestExecRunnerPreflightPinsDriverAndRefusesWritableRunnerSource(t *testing.T) {
	...
	if err := os.Chmod(request.RunnerDirectory, 0o770); err != nil { //nolint:gosec // test runner directory permission
		t.Fatal(err)
	}
	if err := runner.Preflight(config); err == nil {
...
func TestOpenLogModesAndRedactionLimits(t *testing.T) {
	...
	_ = appendFile.Close()
	if err := os.Chmod(path, 0o666); err != nil { //nolint:gosec // test log file permission
		t.Fatal(err)
	}
...
func writeDriver(t *testing.T, content string) (string, string) {
	...
	if err := os.WriteFile(path, []byte(content), 0o700); err != nil { //nolint:gosec // test helper script needs execute bits
		t.Fatal(err)
	}
