...
func (client *GitHubClient) DispatchWorkflow(
	...
	if status != http.StatusOK {
		...
		return DispatchReceipt{}, fmt.Errorf("%w: %v", ErrAmbiguous, remote)
	}
	var response struct {
...
func (client *GitHubClient) DeleteRunner(ctx context.Context, repository string, runnerID int64) error {
	...
	return fmt.Errorf("%w: %v", ErrAmbiguous, &HTTPError{Operation: "delete runner", Status: status, RequestID: requestID})
}
...
func (client *GitHubClient) GenerateJIT(
	...
	if status != http.StatusCreated {
		...
		return JITRegistration{}, fmt.Errorf("%w: %v", ErrAmbiguous, remote)
	}
	var response struct {
...
func (client *GitHubClient) CancelRun(ctx context.Context, repository string, runID int64) error {
	...
	if status != http.StatusAccepted {
		...
		return fmt.Errorf("%w: %v", ErrAmbiguous, remote)
	}
	return nil
}
