...
func (dispatcher *Dispatcher) Dispatch(ctx context.Context, request Request) (Result, error) {
	...
	if ledger.CleanupComplete {
		...
		if releaseErr != nil {
			...
			return dispatcher.result(&ledger), errors.Join(dispatchErr, fmt.Errorf("%w: guard lease release failed: %w", ErrAmbiguous, releaseErr))
		}
	}
	...
	if err != nil {
		if authoritativeJITRejection(err) {
			...
			if saveErr := dispatcher.deps.Store.Save(*ledger); saveErr != nil {
				err = errors.Join(err, fmt.Errorf("%w: persist authoritative JIT rejection: %w", ErrAmbiguous, saveErr))
			}
		}
...
func (dispatcher *Dispatcher) run(
	...
	if err := dispatcher.transition(ledger, StatusCompleted, ""); err != nil {
		ledger.CleanupComplete = false
		return dispatcher.result(ledger), fmt.Errorf("%w: persist completed run: %w", ErrAmbiguous, err)
	}
...
func (dispatcher *Dispatcher) finishError(ledger *Ledger, code string, cause error) (Result, error) {
	...
	if cleanupErr != nil {
		ledger.CleanupComplete = false
		cause = errors.Join(cause, fmt.Errorf("%w: cleanup remains unresolved: %w", ErrAmbiguous, cleanupErr))
